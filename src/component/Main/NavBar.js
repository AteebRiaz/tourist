...
const NavBar = () => {
  ...
  const placeholderTexts = [
    '          where you want to go for your honey moon',
    '          pakistan is the most beautiful country in the words',
    '          i want to go nothern sides of pakistan it is very beautiful',
  ];
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (typingIndex < currentText.length) {
        ...
        // Typing animation for the current text is complete
        clearInterval(interval);
        setTimeout(() => {
          // Delay before starting a new animation
          setTypingIndex(0); // Reset typing index
          setCurrentTextIndex((prevIndex) => (prevIndex + 1) % placeholderTexts.length);
        }, 1000); // Adjust the delay before starting the next animation (1 second in this example)
      }
          ...
          <div className="row justify-content-center py-5">
            <div className="col-lg-10 pt-lg-5 mt-lg-5 text-center">
              ...
              <div className="position-relative w-75 mx-auto animated slideInDown">
                <input
                  className="form-control border-0 rounded-pill w-100 py-3 ps-4 pe-5 typing-animation"
                  type="text"
